...
export const CourseList : React.FC<{openCourse:null|number,published:number,setOpenCourse: React.Dispatch<React.SetStateAction<null|number>>}> = (props) => {
  ...
  const [ orderCourses, setOrderCourses] = useState(false)
  const getListCourses = async () => {
    try{
      const filter = {"status":1,"published":props.published,order:"ASC",orderedBy:"order"}     
  ...
  useEffect(()=>{ getListCourses() },[props.openCourse,orderCourses])
  return(
    <div className="flex flex-col w-full">
      <div className="flex justify-between">
        ...
        {props.published == 1 && <Button name="Reordenar Cursos" btn="success" type="notline" icon="faSortNumericAsc" onClick={()=>setOrderCourses(true)}/>}
      </div>
      <div className="overflow-hidden p-2 rounded flex flex-wrap bg-neutral-950/50 rounded-md">
        ...
        : <>
          ...
          {orderCourses && <CourseOrder courses={listCourses} close={setOrderCourses}/> }
          </>}
      </div>
