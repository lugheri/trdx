...
export const ListCourses : React.FC<{setOpenCourse: React.Dispatch<React.SetStateAction<null|number>>}> = (props) => {
  ...
  useEffect(()=>{ getListCourses() },[])
  return(
    <div className="flex flex-col">
      <p className="font-bold text-neutral-400 my-2">
        {totalCourses} Curso(s) Cadastrado(s).
      </p>
      <div className="overflow-hidden p-2 rounded flex flex-wrap">
        ...
        : listCourses.map((course,key)=>
          <div key={key} 
            ...
            <div className="bg-neutral-950/40 w-full h-[180px] flex justify-center items-center relative overflow-hidden">
              <RenderImageGallery className="w-full h-full" imageId={course.image}/>
              { course.published == 1 ? 
