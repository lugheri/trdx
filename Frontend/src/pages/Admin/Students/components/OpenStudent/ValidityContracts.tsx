...
export const ValidityContract : React.FC<IValidityContractComponent> = (props) => {
  ...
  return(
    <Modal component={
      <div>
        <TitleModal icon='faCalendarCheck' close={()=>props.close(null)} title='Contratos de Vigência'
               subtitle={`Planos de Vigência no curso ${props.courseName} do aluno ${props.studentName}`}/>
...
export const NewValidityContract : React.FC<INewValidityContract> = (props) => {
  ...
  useEffect(()=>{
    if(startContract){
      ...
      const end = cycle == "A" ? init.add(1, 'years').format('YYYY-MM-DD')
                ...
                : cycle == "V" ? init.format('YYYY-MM-DD')
                : ""
                console.log('END',end)
      setEndContract(end)
    }
  ...
  return(
    <Modal component={<div>
      <TitleModal icon='faCalendarPlus' close={()=>props.close(null)} 
          title='Novo Contrato de Vigência'
          subtitle={`Crie um novo Planos de Vigência no curso ${props.infoCourse.name} do aluno ${props.studentName}`}/>
...
export const RemoveValidityContract : React.FC<IRemoveValidityContract> = (props) => {
  ...
  return(
    <Modal component={
      <div>
        <TitleModal
          icon='faCalendarXmark' close={()=>props.close(null)} 
          title='Remover Contrato de Vigência'
          subtitle={`Remover Plano de Vigência no curso ${props.infoCourse.name} do aluno ${props.studentName}`}/>
        <div>
