...
export const NextLesson : React.FC<NextLessonComponent> = (props) => {
  ...
  return(
    <>
     { nextLessonId ? <InfoNextLesson lessonId={nextLessonId}/> : <p>Iniciar</p> } 
    </>
  )
}
...
const InfoNextLesson : React.FC<{lessonId:number}> = (props) => {
  ...
  return (
    <>
      { infoLesson &&
        <div className="flex flex-col mt-[100px] justify-center items-center ml-20 mr-4
                        lg:flex-row xl:flex-row 2xl:flex-row lg:ml-28 xl:ml-28 2xl:ml-28">
          <div className="w-[500px] h-[270px] overflow-hidden mb-4 rounded-xl relative flex justify-center items-center bg-slate-900 text-neutral-600">
            <p className="p-2 bg-neutral-800 text-white text-xs font-light rounded-md absolute top-4 left-4 shadow">
              Continue de onde parou 
            </p>
            { infoCourse ? <RenderImageGallery className="w-full h-full" imageId={infoCourse.image}/>
            : <p>CAPA</p>}
          </div>
          <div className="flex flex-col flex-1 justify-start items-center md:items-start px-4 mx-8">
            <p className="text-neutral-100 font-bold text-xl w-full text-center md:text-left">{infoLesson.name}</p>
            <p className="text-neutral-100 font-light my-4 w-full text-center md:text-left">{infoLesson.description ? infoLesson.description: infoCourse?.description}</p>
